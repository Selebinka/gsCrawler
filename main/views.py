...
def search_result(request):
    ...
    return render(request, 'result_list.html', {'result_filter': result_filter})

def manager_spider(request):
    form = RunCrawlForm()
    return render(request, 'manager_spider.html', {'form': form})
...
def crawl_run(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            search_result = GoogleResultModel.objects.all().order_by('-date')[:value]
            return render(request, 'result_crawl.html', {'search_result': search_result})
